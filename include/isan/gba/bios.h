...
void svcSoftReset(void) IS_NORETURN;
void svcRegisterRamReset(u32 flags);
void svcHalt(void);
void svcStop(void);
void svcIntrWait(bool waitNext, u32 mask);
void svcVBlankIntrWait(void);
s32 svcDiv(s32 num, s32 den); // todo: add a way to read r1 (modulo result)
s32 svcDivArm(s32 den, s32 num); // todo: add a way to read r1 (modulo result)
u16 svcSqrt(u32 num);
...
void svcHardReset(void) IS_NORETURN;
